...
This script modifies the appearance of various widgets we've previously encountered.

.. code-block:: python
    :caption: scripts/PYQT3/04_widget_customized/01_many_widget/main.py
...
Styles for each widget are defined within the ``apply_styles()`` method. For example, the ``QLineEdit`` widget is given a black background, yellow text, an orange border, and its edges are rounded.
