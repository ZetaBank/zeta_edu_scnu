...
For our people detection task, we use NVIDIA built python program called the ``detectnet.py``.
...
-   Within our ``tao_peoplenet.ipynb``, we can see that we first initialize the *PATH* for our input and output images.
...
-   When we execute our python program (detectnet.py), we can see the three parameters the user has set up
    ...
    1.  ``--network=peoplenet``: to specify which network we wished to use.
